...
\[
z_t \equiv [z_t^{(1)}, \dots, z_t^{(S)}], \qquad z_t^{(j)} \in \mathbb{R}^{B \times T \times N \times d}.
\]
...
The active slot for each scale at phase \(\phi\) is:
\[
a(\phi,s)=\mathrm{offset}(s)+(\phi \bmod s).
\]

| phase \(\phi\) | active slot for \(s=1\) | active slot for \(s=2\) | active slot for \(s=4\) |
|---:|---:|---:|---:|
| 0 | 0 | 1 | 3 |
| 1 | 0 | 2 | 4 |
| 2 | 0 | 1 | 5 |
...
flowchart TB
  ...
  subgraph phi1["phase φ=1 (shift by 2)"]
    direction LR
    S0b["S0"] --> H2b["H2"]
    S1b["S1"] --> H3b["H3"]
    S2b["S2"] --> H0b["H0"]
  ...
  subgraph phi2["phase φ=2 (shift by 0)"]
    direction LR
    S0c["S0"] --> H0c["H0"]
    S1c["S1"] --> H1c["H1"]
    S2c["S2"] --> H2c["H2"]
  ...
  subgraph phi3["phase φ=3 (shift by 2)"]
    direction LR
    S0d["S0"] --> H2d["H2"]
    S1d["S1"] --> H3d["H3"]
    S2d["S2"] --> H0d["H0"]
...
| phase \(\phi\) | active slots (s=1, s=2, s=4) |
|---:|:---|
| 0 | (0, 1, 3) |
| 1 | (0, 2, 4) |
| 2 | (0, 1, 5) |
...
| phase \(\phi\) | stream 0 | stream 1 | stream 2 | stream 3 |
|---:|:---|:---|:---|:---|
