...
# Unoptimized implementation of MORPHER that uses Einsums
def lcm(a, b):
    return a * b // gcd(a, b)
...
class Morpher(nn.Module):
    ...
    def __init__(
        ...
        assert stream_head_assignment in StreamHeadAssignment, (
            f"Invalid stream_head_assignment: {stream_head_assignment}"
        )
        ...
        self.encoder = StreamLoRAEncoder(
            input_dim=io_dim, output_dim=self.D, num_splits=self.N, rank=enc_dec_rank
        )
        ...
        self.decoder = StreamLoRADecoder(
            num_splits=self.N, input_dim=self.D, output_dim=io_dim, rank=enc_dec_rank
        )
    ...
    def _build_head_assignments_table(self):
        ...
        for scale_idx, s in enumerate(self.time_scales):
            ...
            for j in range(
                s
            ):  # scale 1 has 1 slot, scale 2 has 2 slots, scale 4 has 4 slots, etc.
                if self.stream_head_assignment == StreamHeadAssignment.PRIVATE:
                    ...
                    raise ValueError(
                        "stream_head_assignment must be 'private' or 'shared'"
                    )
    ...
    def step(self, z: torch.Tensor, t: int, is_causal: bool):
        ...
        if self.head_input_scope == HeadInputScope.STREAM:
            ...
            if self.head_input_scope == HeadInputScope.SCALES:
                ...
                queries = torch.einsum(
                    "btnkh,nkhd->bnktd", z_attn, Wq
                )  # [B, N, K, T, d]
                values = torch.einsum(
                    "btnkh,nkhd->bnktd", z_attn, Wv
                )  # [B, N, K, T, d]
